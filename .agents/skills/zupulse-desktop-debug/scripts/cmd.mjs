...
if (existsSync(windowClosedPath)) {
  console.log(JSON.stringify({ ok: false, error: "renderer window closed; restart the driver (userdata persists)" }));
  process.exit(1);
}
...
for (;;) {
  try {
    ...
    if (existsSync(windowClosedPath)) {
      console.log(
        JSON.stringify({ ok: false, error: "renderer window closed; restart the driver (userdata persists)" }),
      );
      process.exit(1);
    }
