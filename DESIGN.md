...
- **颜色有职责**：信号橙红表达主操作和关键激活；语义色表达状态；signal 色只做音乐编码。
...
- 暖中性灰（浅色主题为 TE 机身浅灰）是基础表面，信号橙红（TE vermilion）是唯一品牌主强调色。
- 信号橙红实心填充只用于真正的主操作（播放、导入）。active/pressed 等开启状态一律用
  `accent` 描边 + `--accent-soft` 底色 + 前景着色的组合，与主操作可区分。
- 信号橙红的透明度派生（soft、glow、focus ring、搜索高亮）一律用 `color-mix` 自
  `--accent-primary` 计算，不写第二个 accent 字面值；`--transport-key-bg` 与 `--meter-fill`
  直接引用 `--accent-primary`。
- 绿、黄、红和蓝只承担明确的状态语义。
- signal blue、pink、purple、yellow 仅用于轨道、节拍、分层和参数编码。
- 同一界面同时使用的 signal 色通常不超过两种，且视觉权重低于信号橙红。
...
- signal 色编码：Viewer = blue、Studio = purple、Library = pink；只出现在编号与指示点等小面积位置，
  视觉权重低于信号橙红。Home 同屏使用三种 signal 色，是“同一界面 signal 色通常不超过两种”的显式例外。
- 全页只有“打开曲库”一个信号橙红实心主操作（位于 Hero）；工作区分区为纯介绍内容，不再各设入口链接。
