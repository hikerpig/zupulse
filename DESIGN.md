...
- 适用的钢琴谱可从练习设置或 Transport 右侧工具区的琴键引导按钮打开琴键引导。它紧贴在乐谱下方、Transport 上方，
  以提示块长度表达
  时值并按同一手色高亮当前发声键；正在发声的提示块加亮，coral 只用于击打线。关闭后完整归还谱面空间。
  预览窗口跟随当前速度固定提前约 2 秒，慢速练习不局促、快速乐段不拥挤。
  左右手使用不同 signal 色但同时保留文字图例，
...
- `.design_library/tab-viewer-te-braun-theme` 是已采用的可切换并行主题（Device Theme，status: adopted），
  通过 `:root[data-shell="device"]` 在 tokens.css 与各 CSS Module 未分层段覆写运行时表现；
  classic 仍为缺省外壳，两个 theme library 与本文件的一致性锁同时生效。
...
- 并行主题（Device Theme）：`.design_library/tab-viewer-te-braun-theme/README.md`
