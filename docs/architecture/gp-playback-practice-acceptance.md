...
仓库当前只有一个已确认可用于测试的候选样本 `test-fixtures/gp/Treasure.gp5`，以及由它确定性派生的 `test-fixtures/gp/generated/desktop-acceptance.gp`；尚未留下人工验收记录，也不足以覆盖完整格式矩阵。下列未验证项目保持“未执行”，不能用自动解析结果替代真实文件结论。
