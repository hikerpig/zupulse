...
## 当前钢琴 development 基线（2026-09-05）

本节索引 `7cef7bc1` 的已提交证据，不改写前文 frozen historical results，也不代表新的模型重跑。

| 范围        | 已验证结果                                                                  | 边界                                             |
| ----------- | --------------------------------------------------------------------------- | ------------------------------------------------ |
| 钢琴 layout | `piano-grand-staff-v1` 在 K331 六页产出 27 个 2-staff crops，双跑 hash 一致 | 显式 CLI 选项；默认 fragmented auto 不变         |
| 钢琴识别    | Rokot L/M/K：Joint F1 `0.3768`，小节完全正确 `57/274`                       | 与既有 header-context 对照一致，没有额外识别增益 |
| LEGATO      | 本轮没有 joined 27-system F1                                                | 不以 process success 替代质量                    |
| 研究范围    | OLiMPiC 3-staff layout 训练线关闭                                           | 不据此声称扫描钢琴 layout 已解决                 |
| 产品        | 质量证据仍为 `STOP`                                                         | 不批准 Library 导入或 Desktop default 变更       |

唯一完整结果与复现入口：
[K331 piano-grand-staff-v1 report](../../tools/pdf-omr-cli/reports/exploratory/k331-piano-grand-staff-segmentation-v1/README.md)。
当前接口以 `tools/pdf-omr-cli/src/staff-system-segmentation.ts` 为准；已完成 Spec 从工作树移除。
后续执行状态只放在 `tasks/`，本页不维护任务步骤。

后续 LEGATO 归因、候选收益与拒绝边界统一见
[LEGATO 优化阶段总结](../../tools/pdf-omr-cli/reports/exploratory/legato-optimization-summary-20260915.md)。

## 指标解释

- `topology-exact` 检查 system 数量、顺序、中心落在真值纵带以及声明的 staff count，不证明裁切完整性。
- `validMeasure` 是按 joint 条件与真值完全匹配的 staff-measure 比例，不是单独的节奏合法率。
- Pitch、Onset、Duration 分别匹配；Voice F1 也包含 pitch/onset/duration 条件，不能单凭它与 Joint F1 接近断言声部是主因。
- 结构合法、可解析、可重复与 process success 均不能替代识别质量。K331 exploratory symbolic evaluation 与
  readiness-gated benchmark 是不同口径，不能据此生成 Harmony delta 或改写 frozen gate。

定义以
[symbolic metrics](../../tools/pdf-omr-cli/src/benchmark/symbolic-metrics.ts)、
[piano topology](../../tools/pdf-omr-cli/src/piano-grand-staff-topology.ts) 为准。

## 相关历史证据

以下报告只解释已完成实验，不是下一轮执行指令。完整数字只保留在对应报告，不在本页重复展开。

| 问题                             | 证据                                                                                                                                                                                                           |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| K331 输入粒度与 L/M/K 收益       | [page-scope ablation](../../tools/pdf-omr-cli/reports/exploratory/k331-page-scope-ablation-v1/README.md)                                                                                                       |
| Header 策略不能按 K331 单曲推广  | [K331 ablation](../../tools/pdf-omr-cli/reports/exploratory/rokot-header-context-ablation-v1/README.md)、[DCML 对照](../../tools/pdf-omr-cli/reports/exploratory/rokot-header-context-dcml-piano-v1/README.md) |
| 下一轮已有钢琴语料与限制         | [corpus suitability](../../tools/pdf-omr-cli/reports/exploratory/piano-corpus-suitability-v1/README.md)                                                                                                        |
| 非 fragmented 模式的选择依据     | [K331 layout](../../tools/pdf-omr-cli/reports/exploratory/k331-piano-grand-staff-layout-v1/README.md)                                                                                                          |
| 真实扫描布局与 multi-system 失败 | [detector v2](../../tools/pdf-omr-cli/reports/development/olimpic-full-page-detector-v2/README.md)、[real multi-system](../../tools/pdf-omr-cli/reports/development/olimpic-real-multisystem-v1/README.md)     |
| LEGATO extra parts 不能安全丢弃  | [topology audit](../../tools/pdf-omr-cli/reports/exploratory/olimpic-legato-topology-audit-v1/README.md)                                                                                                       |

LEGATO 外部补白、重复惩罚消融和上缘恢复均未形成可采用的通用收益；八度谱号诊断没有交付 detector，
不能作为识别的前置条件。重复中间报告已合并到上述阶段总结，历史恢复方式也在该入口说明。

已关闭 layout 训练的代码与实验演变通过 Git 历史追溯；不恢复为当前规范。
