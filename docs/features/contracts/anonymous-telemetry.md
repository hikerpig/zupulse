...
- Provider adapter 只向 PostHog Cloud US 的精确 ingestion origin 发送 allowlisted properties，禁用
  person profile 与 GeoIP enrichment；无效配置和 provider failure 降级为 No-op。`web-core` 只构造 allowlisted
  envelope；Browser 与 Desktop 各自注入 PostHog HTTP transport。Telemetry Preference 与 Anonymous Installation
  仍由各宿主拥有。
...
| ----------------------------- | -------------------- | -------------------------- | ---------------------------------------------------------- |
| 匿名 semantic events          | 支持（有效分发配置） | 支持（Main-owned context） | envelope 共享；HTTP transport 与 Preference 分宿主         |
