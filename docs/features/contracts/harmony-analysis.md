...
implementation_paths:
  ...
  - apps/desktop-shell/src/main/library/desktop-library-store.ts
supersedes: []
---
...
桌面宽屏使用可调节的乐谱/分析左右分栏；窄于或等于 960 px 时改为固定工作区内的上下两行，
...
仍会临时显示。结构化和弦构建器只向用户显示本地化的和弦类型与度数操作名称，持久化和命令参数
继续使用既有领域枚举值。
...
分析结果重新显现；N.C. 只能来自明确的来源或 User Correction。MusicXML `<numeral>`、不支持的
`kind` 等无法表达为产品 Chord Symbol 的来源 `<harmony>` 只保留为诊断证据，不进入 Effective
Projection 的边界集合，也不覆盖 active Analysis Revision。
...
5. Effective Projection 的优先级固定为 User Correction、supported source harmony、active Revision；
   unsupported source harmony 不属于 Effective Harmony fact，不得切分或覆盖 Revision。
...
- 给定来源和弦、Revision 与重叠 Correction，当形成有效视图时，必须按 Correction、source、
  Revision 的顺序取值。
- 给定无法表达为 Chord Symbol 的来源 `<harmony>` 与重叠 Revision，当形成有效视图时，该来源标记
  不得产生边界或覆盖 Revision；其诊断表示不得被误当作算法 `low-confidence`。
...
- 给定两个 session 基于同一 document version 保存，当后一个遇到 CAS 冲突时，不得覆盖先提交的
  Document。
- 给定 Studio 宽度不超过 960 px，乐谱与分析 pane 必须同时位于固定工作区内且各自可滚动；给定
  紧凑桌面高度，片段编辑器底部操作必须可通过其滚动容器到达。
- 给定筛选结果少于列表可容纳行数，片段行必须保持紧凑内容高度并从列表顶部排列，不得平均拉伸
  填满剩余高度；给定结果超过可用空间，列表必须保持可滚动。
- 给定用户打开片段试听设置，和弦预览必须显示为固定“和弦预览”标签与 switch 状态；切换后必须立即
  更新谱面临时和弦标注，不能依赖会随状态变化的动作按钮文案表达当前状态。
- 给定 Document 存在 unresolved range，当片段工作区首次挂载时必须默认显示“待确认”；给定不存在
  unresolved range 时必须默认显示“全部”。用户切换筛选后不得因数据刷新被自动改写。
- 给定中文或英文界面，当用户打开结构化和弦构建器时，option label 必须使用当前 locale 的产品
  术语，同时提交的 chord kind 与 degree operation 仍必须是领域 schema 定义的原始枚举值。
...
| Effective Projection 与 Corrections       | [`effectiveProjection.ts`](../../../packages/web-core/src/harmony/effectiveProjection.ts)、[`sourceHarmony.ts`](../../../packages/web-core/src/harmony/sourceHarmony.ts)、[`correctionCommands.ts`](../../../packages/web-core/src/harmony/correctionCommands.ts)                                                                                          | [`effectiveProjection.test.ts`](../../../packages/web-core/src/harmony/__tests__/effectiveProjection.test.ts)、[`sourceHarmony.test.ts`](../../../packages/web-core/src/harmony/__tests__/sourceHarmony.test.ts)、[`correctionCommands.test.ts`](../../../packages/web-core/src/harmony/__tests__/correctionCommands.test.ts)、Browser K331 E2E                                 |
...
| Studio 响应式、初始筛选与本地化编辑       | [`studio-split-workspace.module.css`](../../../packages/web-viewer/src/components/studio-split-workspace.module.css)、[`harmony-range-workspace.tsx`](../../../packages/web-viewer/src/features/harmony-studio/harmony-range-workspace.tsx)、[`HarmonyStudioEditor.tsx`](../../../packages/web-viewer/src/features/harmony-studio/HarmonyStudioEditor.tsx) | [`styles.test.ts`](../../../packages/web-viewer/src/__tests__/styles.test.ts)、[`harmony-range-workspace.test.tsx`](../../../packages/web-viewer/src/features/harmony-studio/__tests__/harmony-range-workspace.test.tsx)、[`HarmonyStudioEditor.test.tsx`](../../../packages/web-viewer/src/features/harmony-studio/__tests__/HarmonyStudioEditor.test.tsx)、Browser Studio E2E |
...
| Studio alphaTab chord preview             | [`alpha-tab-studio.ts`](../../../packages/web-core/src/gp/alpha-tab-studio.ts)、[`studio-score-runtime.ts`](../../../packages/web-viewer/src/studio-score-runtime.ts)                                                                                                                                                                                      | [`alpha-tab-studio.test.ts`](../../../packages/web-core/src/gp/__tests__/alpha-tab-studio.test.ts)、Browser K331 E2E                                                                                                                                                                                                                                                            |
| Browser/Desktop 持久化与删除联动          | [`indexed-db-sheet-library-repository.ts`](../../../packages/web-storage/src/indexed-db-sheet-library-repository.ts)、[`desktop-library-store.ts`](../../../apps/desktop-shell/src/main/library/desktop-library-store.ts)                                                                                                                                  | 双宿主 Repository tests                                                                                                                                                                                                                                                                                                                                                         |
| Effective Projection 的 MusicXML/MXL 导出 | [`exportMusicXmlHarmony.ts`](../../../packages/web-core/src/harmony/exportMusicXmlHarmony.ts)、[`musicXmlRoundTrip.ts`](../../../packages/web-core/src/harmony/musicXmlRoundTrip.ts)、[`harmonyStudioExport.ts`](../../../packages/web-viewer/src/harmonyStudioExport.ts)                                                                                  | [`exportMusicXmlHarmony.test.ts`](../../../packages/web-core/src/harmony/__tests__/exportMusicXmlHarmony.test.ts)、[`harmonyStudioExport.test.ts`](../../../packages/web-viewer/src/__tests__/harmonyStudioExport.test.ts)、Browser K331 E2E                                                                                                                                    |
