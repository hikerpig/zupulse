...
implementation_paths:
  ...
  - packages/web-viewer/src/features/sheet-library
...
Browser import modal 还接受 Web `File` 拖放；Desktop 通过 Preload `webUtils.getPathForFile` +
Main 一次性 token 接受 Finder 拖放，两端都把 picker 与 dropped files 归一为相同的
`ScoreImportSource`。两端拖放都使用与原生多选相同的入口扩展名筛选与 size 上限规则，不支持
的文件不发 token、不读 IPC 字节，仅在 modal 内显示 skipped 状态。
...
picker 与 dropped files 在进入候选列表前使用 Library 专用扩展名规则筛选，只接受 GP 和
MusicXML/MXL；包括 MIDI 在内的其他文件被跳过，并在 modal 内显示本次选择的 skip status。被跳过
的文件不会读取字节、不会提交到 import pipeline，也不会生成 `ImportItemResult` 或进入导入完成后的
batch summary。该入口筛选不替代领域防线；绕过 modal 直接提交的不支持格式仍返回
`UNSUPPORTED_FORMAT`。

每份通过入口筛选的候选文件独立执行以下流程：
...
批量导入只汇总实际提交的候选，并通过逐项结果隔离失败：无效文件或绕过入口筛选的不支持格式不会
阻止其他文件导入。结果类型定义的失败 code 包括 `FILE_TOO_LARGE`、`UNSUPPORTED_FORMAT`、`INVALID_SCORE`、`READ_FAILED`、
...
| 导入文件拖放          | Web `File`                   | Preload `webUtils` + Main 一次性 token，不绕过 Main/token 边界 | 双平台共享候选流程                  |
...
- 给定 picker 或 drop 同时包含可导入曲谱与 MIDI/其他不支持文件，当 modal 建立候选时，只保留
  GP 与 MusicXML/MXL，显示 skip status，且被跳过文件不进入导入完成后的 batch summary。
...
| Desktop 托管文件、练习摘要、SQLite 状态和 reconciliation  | [`desktop-library-store.ts`](../../../apps/desktop-shell/src/main/library/desktop-library-store.ts)、[`library-module.ts`](../../../apps/desktop-shell/src/main/library/library-module.ts)、[`reconcile.ts`](../../../apps/desktop-shell/src/main/library/reconcile.ts)                                                                                                                                                                                                                                                                 | [`desktop-library-store.test.ts`](../../../apps/desktop-shell/src/main/library/__tests__/desktop-library-store.test.ts)、[`library-module.test.ts`](../../../apps/desktop-shell/src/main/library/__tests__/library-module.test.ts)、[`reconcile.test.ts`](../../../apps/desktop-shell/src/main/library/__tests__/reconcile.test.ts)、[`desktop.spec.ts`](../../../apps/desktop-shell/e2e/desktop.spec.ts)                                                                                                                                                          |
| Library 目录、练习动作、过滤、导入反馈与管理菜单          | [`sheet-library`](../../../packages/web-viewer/src/features/sheet-library)                                                                                                                                                                                                                                                                                                                                                                                                                                                              | [`SheetLibrary.test.tsx`](../../../packages/web-viewer/src/features/__tests__/SheetLibrary.test.tsx)、[`sheet-library/__tests__`](../../../packages/web-viewer/src/features/sheet-library/__tests__)、[`App.test.tsx`](../../../packages/web-viewer/src/app/__tests__/App.test.tsx)、[`library.spec.ts`](../../../apps/web-demo/e2e/library.spec.ts)                                                                                                                                                                                                               |
| Modal 候选审阅、Library 格式筛选与双平台 dropped files    | [`ImportScoreDialog.tsx`](../../../packages/web-viewer/src/features/ImportScoreDialog.tsx)、[`importLibraryScores.ts`](../../../packages/web-core/src/library/importLibraryScores.ts)、[`BrowserScoreFileGateway.ts`](../../../apps/web-demo/src/library/BrowserScoreFileGateway.ts)、[`preload.ts`](../../../apps/desktop-shell/src/preload.ts)、[`score-files.ts`](../../../apps/desktop-shell/src/main/file-access/score-files.ts)、[`desktop-score-file-gateway.ts`](../../../apps/desktop-shell/src/desktop-score-file-gateway.ts) | [`SheetLibrary.test.tsx`](../../../packages/web-viewer/src/features/__tests__/SheetLibrary.test.tsx)、[`importLibraryScores.test.ts`](../../../packages/web-core/src/library/__tests__/importLibraryScores.test.ts)、[`BrowserScoreFileGateway.test.ts`](../../../apps/web-demo/src/library/__tests__/BrowserScoreFileGateway.test.ts)、[`desktop-score-file-gateway.test.ts`](../../../apps/desktop-shell/src/__tests__/desktop-score-file-gateway.test.ts)、Desktop E2E dropped-files isolation、[`library.spec.ts`](../../../apps/web-demo/e2e/library.spec.ts) |
| Bundled sample catalog、hash 与双宿主普通馆藏生命周期     | [`sample-scores.ts`](../../../packages/web-viewer/src/sample-scores.ts)、[`manifest.json`](../../../product-assets/samples/manifest.json)                                                                                                                                                                                                                                                                                                                                                                                               | [`verify-sample-scores.test.ts`](../../../scripts/__tests__/verify-sample-scores.test.ts)、[`sample-scores.test.ts`](../../../packages/web-viewer/src/__tests__/sample-scores.test.ts)、Browser/Desktop E2E 与 package verifier                                                                                                                                                                                                                                                                                                                                    |
...
| Desktop Renderer 不获得文件路径且候选审阅不依赖 token TTL | Bridge schemas、Main handler、[`desktop-score-file-gateway.ts`](../../../apps/desktop-shell/src/desktop-score-file-gateway.ts)                                                                                                                                                                                                                                                                                                                                                                                                          | Desktop Bridge tests、[`desktop-score-file-gateway.test.ts`](../../../apps/desktop-shell/src/__tests__/desktop-score-file-gateway.test.ts)、Desktop E2E isolation test                                                                                                                                                                                                                                                                                                                                                                                             |
