...
- 每节一个文字级入口（非实心按钮），导向对应工作区的实际起点。Viewer 与 Studio 都需要
  先有曲谱，因此入口统一导向 `/library` 并在文案中说明"从曲库选择一份曲谱"。
...
- 暖中性灰表面体系不变；珊瑚色仍是唯一主强调色，且首页只有"打开曲库"一个实心主操作；
  各区文字入口使用前景色 + 细下划线，hover/focus 时才过渡到珊瑚色，不把可点击元素都染成珊瑚色。
...
- 交互态：所有入口可 Tab 到达，有可见 focus 样式；主操作与文字入口权重明确区分。
