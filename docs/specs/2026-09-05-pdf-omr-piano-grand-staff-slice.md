...
OLiMPiC 6 works / 29 页全是人声+钢琴 3-staff mixed，不是产品分母。Rokot 合同是 1–2 staff。为扫描 3-staff
再训练 layout 已经结案，不进入 Desktop。产品路径是钢琴练习、大谱表。
...
3. **先测后训。** 只用现有 `rokot-staff-system-v2`，全局参数冻结，禁止按页路由。
...
- pairing-only：最小 grand-staff pairing 修复，不训练。
- layout 已够、F1 仍低：下一投资是 2-staff 识别，不是 layout。

## 2026-09-05 implementation outcome

测量完成，未训练。K331 六页 truth 为 `6/6/1/6/6/2`。runtime fragmented 最多 2/6；non-fragmented
grand-staff 为 6/6。CLI identity `piano-grand-staff-v1` 已落地；Rokot header-context Joint F1 `0.3768`。
产品 runtime 保持 `STOP`。
