...
该功能采用“规则候选生成 + 本地学习型候选重排序 + 全局序列解码 + 非和弦音修正 + 置信度拒识”的轻量混合架构。学习模型作为版本化只读资产随应用发布，离线且确定性运行；它不改变现有 Managed Score Copy 不可变、Viewer 负责查看与练习的边界。
...
- 端到端神经网络或在线模型服务；首版仅允许 ADR 0053 定义的本地轻量候选重排序与拒识。
