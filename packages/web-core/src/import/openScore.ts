...
export async function openScore(input: OpenScoreInput): Promise<ImportResult> {
  ...
  try {
    const output = await adapter.parse({ fileName: input.fileName, bytes: input.bytes, ...(input.signal ? { signal: input.signal } : {}) });
    ...
    const session = await createViewerSession({
      fileName: input.fileName, bytes: input.bytes, format: probe.format,
      capabilities: input.capabilities, runtime: output.runtime,
      diagnostics: output.diagnostics, scoreCapabilities: output.capabilities,
    });
    ...
    return { status: output.diagnostics.length ? "success-with-warnings" : "success", candidate, diagnostics: output.diagnostics };
  } catch (cause) {
    if (input.signal?.aborted) throw input.signal.reason ?? new DOMException("Aborted", "AbortError");
...
export async function openScoreThroughBridge(input: {
  ...
  return openScore({ fileName: file.fileName, bytes: file.bytes, capabilities: handshake.capabilities, adapters: input.adapters, ...(input.signal ? { signal: input.signal } : {}) });
}
