...
describe("MusicXML preflight", () => {
  it("extracts root structure and bounded counts", () => {
    expect(preflightMusicXml(xml(`<?xml version="1.0"?><score-partwise version="4.0"><part id="P1"><measure number="1"><note/></measure></part></score-partwise>`)))
  ...
  it("enforces MXL budgets and resolves the declared rootfile", () => {
    const entries = [
      { name: "META-INF/container.xml", uncompressedSize: 100, bytes: xml(`<container><rootfiles><rootfile full-path="score.musicxml"/></rootfiles></container>`) },
