...
export function preflightMusicXml(bytes: Uint8Array): MusicXmlPreflight {
  ...
  if (!new RegExp(`<\\/${root}\\s*>\\s*$`, "i").test(withoutProlog) && !new RegExp(`^<${root}\\b[^>]*/>\\s*$`, "i").test(withoutProlog)) {
  ...
  if (partCount > MUSICXML_LIMITS.maxParts || measureCount > MUSICXML_LIMITS.maxMeasures || noteCount > MUSICXML_LIMITS.maxNotes) {
...
export function preflightMxlEntries(entries: MxlEntry[], limits: MxlLimits = {}): { rootFileName: string; rootBytes: Uint8Array } {
  ...
  if (entries.length > maxEntries || entries.some(entry => entry.uncompressedSize > maxEntryBytes) || entries.reduce((sum, entry) => sum + entry.uncompressedSize, 0) > maxTotal) fail("resource-limit-exceeded");
  ...
  if (!rootFileName || rootFileName.startsWith("/") || rootFileName.split("/").includes("..")) fail("mxl-rootfile-missing");
...
function count(source: string, pattern: RegExp): number { return source.match(pattern)?.length ?? 0; }
function fail(code: ConstructorParameters<typeof ImportPreflightError>[0]): never { throw new ImportPreflightError(code, code); }
