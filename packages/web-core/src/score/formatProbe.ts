...
export type FormatProbeResult =
  | { status: "confirmed"; format: Exclude<ScoreFormat, "midi">; extensionHint?: ScoreFormat; evidence: string; preflight?: MusicXmlPreflight }
...
export async function probeScoreFormat(fileName: string, bytes: Uint8Array): Promise<FormatProbeResult> {
  ...
  if (startsAscii(bytes, "FICHIER GUITAR PRO") || (bytes[0] === 0x50 && bytes[1] === 0x4b && extensionHint === "gp")) {
  ...
  if (/^\s*(?:<\?xml[\s\S]*?\?>\s*)?(?:<!DOCTYPE[\s\S]*?>\s*)?<score-(?:partwise|timewise)\b/i.test(prefix)) {
    try {
      return { status: "confirmed", format: "musicxml", ...optionalHint, evidence: "musicxml-root", preflight: preflightMusicXml(bytes) };
    } catch (error) {
      const malformed = error instanceof ImportPreflightError && error.code === "malformed-score";
      return { status: malformed ? "malformed" : "unsupported", ...optionalHint, diagnostic: createImportDiagnostic(malformed ? "malformed-score" : "unsupported-format") };
    }
  }
