...
export const writtenPositionSchema = z.object({
  schemaVersion: z.literal(1), trackId: z.string().min(1), measureIndex: z.number().int().nonnegative(),
  beatIndex: z.number().int().nonnegative(), tick: z.number().int().nonnegative(),
}).strict();
export const playbackOccurrenceSchema = z.object({
  schemaVersion: z.literal(1), written: writtenPositionSchema, occurrenceIndex: z.number().int().nonnegative(),
  timelineTick: z.number().int().nonnegative(), path: z.array(z.number().int().nonnegative()),
}).strict();
...
export class PositionMap {
  ...
  restore(occurrence: PlaybackOccurrence): PlaybackOccurrence | undefined {
    return this.occurrences.find(item => item.timelineTick === occurrence.timelineTick && item.path.join(".") === occurrence.path.join("."))
      ?? this.occurrencesFor(occurrence.written)[0];
  }
}
function sameWritten(a: WrittenPosition, b: WrittenPosition): boolean {
  return a.trackId === b.trackId && a.measureIndex === b.measureIndex && a.beatIndex === b.beatIndex && a.tick === b.tick;
}
