...
function PlaybackLayout({
  ...
  if (!playback || !state) return disabledPlaybackWorkspace(children);
    ...
    <>
      <section className="transport-bar" aria-label="播放控制">
        ...
        <div className="transport-tools">
          ...
          <p className={`status-chip ${audioTone(state.soundFont)}`}>
            {audioText(state.soundFont)}
          </p>
      ...
      <section className="workspace">
        {children}
        <aside className="practice-panel" aria-label="练习设置">
              ...
              <div className="item-list">
                {view.loops.map((loop) => (
                  <div className="loop-row" key={loop.id}>
                    <button
                      type="button"
                      onClick={() => dispatch({ type: 'select-loop', loopId: loop.id })}
                    >
                      {loop.selected ? '当前' : '选择'}
                    </button>
                    <input
                    ...
                    <input
                      ...
                      aria-label="循环速度百分比"
                      onChange={(event) =>
                        dispatch(loopSpeedCommand(loop.id, event.currentTarget.value))
                      }
                    />
                    <button
                      type="button"
                      onClick={() => dispatch({ type: 'delete-loop', loopId: loop.id })}
                    >
                      删除
                    </button>
                  </div>
              ...
              {view.tracks.map((track) => (
                <div className="track-row" key={track.id}>
                    ...
                    onChange={(checked) =>
                      dispatch({
                        ...
                        trackIds: checked
                          ...
                          : state.trackState.additionalVisibleTrackIds.filter(
                              (id) => id !== track.id,
                            ),
                      })
                    }
                  ...
                  <Check
                    ...
                    checked={track.muted}
                    onChange={(muted) =>
                      dispatch({ type: 'set-track-mute', trackId: track.id, muted })
                    }
                  />
                  <Check
                    label="独奏"
                    checked={track.solo}
                    onChange={(solo) =>
                      dispatch({ type: 'set-track-solo', trackId: track.id, solo })
                    }
                  />
                  <input
                    type="range"
...
function disabledPlaybackWorkspace(children: ReactNode) {
  return (
    <>
      <section className="transport-bar" aria-label="播放控制">
        <div className="transport-actions">
