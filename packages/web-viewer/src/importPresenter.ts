...
export async function presentScoreFile(input: {
  file: DemoFileLike;
  api: AlphaTabApiLike;
}): Promise<DemoState> {
  ...
  if (input.api.settings?.importer) {
    input.api.settings.importer.encoding =
      probe.format === 'gp' ? detectGpEncoding(bytes) : 'utf-8';
    (
      input.api.settings.importer as { mergePartGroupsInMusicXml?: boolean }
    ).mergePartGroupsInMusicXml = false;
    input.api.updateSettings?.();
  }
  if (!loadAlphaTabBytes(input.api, bytes))
    return { status: 'error', message: 'alphaTab 无法加载该文件' };
