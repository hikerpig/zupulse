...
import { correctAlter, sourceScope, type SourceHead } from "./alter";

const root = "tools/pdf-omr-cli/reports/development";
const output = `${root}/legato-alter-correction-v1-20260914`;
...
const baselinePath = (work: string) => `${root}/legato-staff-slots-v1-20260914/${work}/draft.json`;
const files = [
  evidencePath,
  keysPath,
  ...works.map(baselinePath),
  ...["alter.ts", "alter.test.ts", "alter-generate.ts", "alter-evaluate.ts", "key_evidence.py", "key_audit.py"].map(
...
write("protocol", {
  hashes,
  baseline: "staff-slots for both works; isolated from whole-measure candidate replacement",
  modelCalls: 0,
  generationReferenceReads: 0,
  rule: "Traverse all staff measures; full diatonic-count equality, unique within-pitch order, globally compatible onset/x order; source key plus octave-specific accidental scope; union reset and possible tie-carried alterations between consecutive bars; any source ambiguity or predicted tie rejects the staff measure; change alter and soundingMidi only",
  acceptance:
...
for (const work of works) {
  ...
  const rows: {
    ...
    measure: number;
    reason: string;
    changes: ReturnType<typeof correctAlter>["changes"];
    sourceNotes: number;
  ...
  for (const [staff, s] of staves.entries()) {
    ...
    for (const [measure, m] of s.measures.entries()) {
      ...
      carry = scope.carry;
      const result = correctAlter(m, scope.heads);
  ...
  for (const event of restored.parts
    ...
    if (event.type === "note" && original.type === "note") {
      if (original.writtenPitch && event.writtenPitch) {
        event.writtenPitch.alter = original.writtenPitch.alter;
      }
