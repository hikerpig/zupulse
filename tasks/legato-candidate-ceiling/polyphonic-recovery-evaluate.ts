...
const root = "tools/pdf-omr-cli/reports/development",
  output = `${root}/legato-polyphonic-recovery-v2-20260914`;
const cases = ["score-4", "score-9"].map((id) => ({
  id,
  baseline: `${root}/legato-rhythm-tail-v1-20260914/${id}/draft.json`,
  candidate: `${output}/${id}/draft.json`,
  reference: `${root}/piano-recognition-v1-20260905/${id}-baseline-cli/expected.json`,
}));
const hash = (p: string) => createHash("sha256").update(readFileSync(p)).digest("hex");
const paths = [
  "tasks/legato-candidate-ceiling/polyphonic-recovery-evaluate.ts",
...
writeFileSync(
  ...
  canonicalJson({
    hashes,
    acceptance:
      "Across works: at least one recovered primary match. Per work: zero lost primary matches, nonincreasing note/rest FP/FN, unique nonregressing adjusted strict joint F1 and valid measures, both readiness unblocked, successful export and four round-trip checks; no default promotion",
  }),
  { flag: "wx" },
);
...
writeFileSync(
  ...
  canonicalJson({
    ...
    results,
    overallGate: results.every((r) => r.gate) && results.some((r) => r.recovered.length > 0),
  }),
  { flag: "wx" },
);
