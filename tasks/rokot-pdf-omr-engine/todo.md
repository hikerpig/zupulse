...
- [ ] T02 固定 `abc-xml-converter==1.0.1` external-process contract
- [ ] T03 抽取 deterministic PDF rasterizer
- [ ] T04 实现 deterministic piano grand-staff segmentation

### Checkpoint A

- [ ] 所有 runtime mismatch 在 render/inference 前失败
- [ ] converter failure/cancel 有 fake-process coverage
- [ ] render、crop 与 segmentation hashes 可重复
- [ ] 未新增 native dependency、Draft schema 或 error code

## Phase 1: Engine core

- [ ] T05 定义 `RokotSystemBundle` 与严格 ABC envelope validation
...
- [ ] T10 在 K331 fixture 同目录建立 development-only manifest
