...
11. **候选 Top-1 不等于生产 primary。** `top1Accuracy` 使用 `alternatives[0]`，不会反映 MLP reranker 改选的 chord；report `2.6.0` 起应使用 threshold 前 `predictedPrimaryAccuracy`，并把候选排序与最终决策指标分开解释。
