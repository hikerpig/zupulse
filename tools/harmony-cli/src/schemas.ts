...
const structuredRecordCandidateSchema = z
  .object({
    chordIndex: z.number().int().nonnegative(),
    ruleSequenceScore: twoDecimalScoreSchema,
    segmentFeatures: z.array(twoDecimalScoreSchema).length(STRUCTURED_SEGMENT_FEATURE_LENGTH),
  })
  .strict();
const structuredRecordRangeSchema = z
  .object({
    startBoundaryIndex: z.number().int().nonnegative(),
    endBoundaryIndex: z.number().int().positive(),
    durationQuarterNotes: twoDecimalScoreSchema.positive(),
    candidates: z.array(structuredRecordCandidateSchema).min(1).max(8),
  })
  .strict();
const structuredRecordGoldSegmentSchema = z
  .object({
    startBoundaryIndex: z.number().int().nonnegative(),
    endBoundaryIndex: z.number().int().positive(),
...
export const harmonyStructuredRecordsReportSchema = z
  .object({
    schemaVersion: z.literal("1.0.0"),
    command: z.literal("structured-records"),
    split: z.enum(["train", "tune"]),
    featureVersion: z.literal(STRUCTURED_FEATURE_VERSION),
    transitionFeatureVersion: z.literal(STRUCTURED_FEATURE_VERSION),
    groupsSha256: z.string().regex(/^[a-f0-9]{64}$/),
    searchContract: z
      .object({
        boundaryPolicy: z.literal("dense-note-events"),
        spanMode: z.literal("quarter-notes"),
        maxQuarterNotes: z.literal(8),
        topK: z.literal(8),
        supervision: z.literal("contiguous-representable-subpaths-v1"),
      })
      .strict(),
    sources: z.array(
      z
...
export type HarmonyStructuredRecordsReport = z.infer<typeof harmonyStructuredRecordsReportSchema>;
