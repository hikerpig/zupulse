...
from build_openscore_layout_dataset import draw_system_band_mask
...
class LayoutDataset(Dataset[tuple[torch.Tensor, torch.Tensor, torch.Tensor]]):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        system_mask = draw_system_band_mask(annotation, self.size)
...
def main() -> None:
    ...
    train = LayoutDataset(args.dataset_root, train_pages, MODEL_SIZE)
    validation = LayoutDataset(args.dataset_root, validation_pages, MODEL_SIZE)
